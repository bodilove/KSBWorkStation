...
namespace Test.Common
{
    public class MyDB_Sql : IDisposable
    {
        ...
        public MyDB_Sql(SqlConnection  sqlcon)
        {
            _connection = sqlcon;
        }
        ...
        readonly object _lockCommand = new object();
        ...
        {
            __params = new List<SqlParameter>();
            DataTable dt = null;
            try
            {
                if (_reader != null)
                ...
                {
                    _command.Parameters.Clear();
                    if (__params != null && __params.Count != 0)
                        _command.Parameters.AddRange(__params.ToArray());
        ...
        /// <summary></summary>
        private List<SqlParameter> __params = null;
        ...
        public int ExecuteNonQuery(string sql, List<SqlParameter> _params)
        {
            ...
            {
                SQLBAL.WriteLog.dbErrLog("int ExecuteNonQuery(string sql, List<SqlParameter> _params)", ex.ToString());
                result = -1;
            }
