...
namespace Common
{
    public static class SqlHandle
    {
        ...
        public static string dbname = "Utop8_New";
        ...
        /// <returns></returns>
        public static T GetInfo<T>(string tableName, string condition, long ID) where T : new()
        {
            T t = new T();
            DataTable dt = null;
            using (MyDB_Sql mydbUUTDB = new MyDB_Sql(dbserver, dbname, userid, password))
        ...
        public static long AddT<T>(T model, string tableName)
        {
            string columns = "";
            string content = "";
            List<SqlParameter> _parames = new List<SqlParameter>();
        ...
        public static bool ClearTable(string TableName)//清空，id从清空前的id开始
        {
            bool res = false;
            try
            {
        ...
        public static bool TruncateTable(string TableName)//清空，id从1开始
        {
            bool res = false;
            try
            {
...
/// <param name="_parames"></param>
/// <returns></returns>
        public static string SqlCommand<T>(T model, string Type, string tableName, string condition, out List<SqlParameter> _parames,bool IsContiansPrimaryKey)
        {
            _parames = new List<SqlParameter>();
            string sql = "Erro";
            string type1 = Type.ToUpper();
            if (type1 == "DELETE")
            {
                sql = "delete " + tableName + " where 1=1 " + condition;
                return sql;
            }
            ...
            try
            {
                ...
                if (type1 == "ADD")
                {
                    if (IsContiansPrimaryKey)
                    {
                        if (content.Length == 0 || columns.Length == 0) return "Erro";
                        content = content.Remove(0, 1);
                        content = content.Remove(0, content.IndexOf(',') + 1);
                        columns = columns.Remove(0, 1);
                        columns = columns.Remove(0, columns.IndexOf(',') + 1);
                        sql = "insert into " + tableName + "(" + columns + ") values(" + content + ")";
                    }
                    else
                    {
                        if (content.Length == 0) return "Erro";
                        content = content.Remove(0, 1);
                        columns = columns.Remove(0, 1);
                        sql = "insert into " + tableName + "(" + columns + ") values(" + content + ")";
                    }
                ...
                if (type1 == "UPDATE")
                {
                    ...
                    else
                    {
                     ...
                     sql = "update " + tableName + " set " + content + " where 1=1 " + condition;
                    //if (IsContiansPrimaryKey)
                    //{
                    //    if (content.Length == 0 || columns.Length == 0) return "Erro";
                    //    content = content.Remove(0, 1);
            ...
            {
                WriteLog.dbErrLog("SqlCommand", ex.ToString());
                sql = "Erro";
            }
            return sql;
        }
